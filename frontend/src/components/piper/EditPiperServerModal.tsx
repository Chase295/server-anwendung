...
interface EditPiperServerModalProps {
  isOpen: boolean;
  server: PiperServer | null;
  onClose: () => void;
  onSave: (serverId: string, data: { name: string; url: string; description?: string }) => Promise<void>;
  onDelete: (serverId: string) => Promise<void>;
}

export default function EditPiperServerModal({ isOpen, server, onClose, onSave, onDelete }: EditPiperServerModalProps) {
  ...
  const handleDelete = async () => {
    ...
    if (!confirm(`Möchten Sie den Piper-Server "${server.name}" wirklich löschen?\n\nDieser Vorgang kann nicht rückgängig gemacht werden.`)) {
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="z.B. Deutscher Piper-Server mit Thorsten-Stimme"
              rows={2}
