...
interface AddVoskServerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (server: { name: string; url: string; description?: string }) => Promise<void>;
}

export default function AddVoskServerModal({ isOpen, onClose, onAdd }: AddVoskServerModalProps) {
  const [name, setName] = useState('');
  const [url, setUrl] = useState('ws://localhost:2700');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onAdd({ name: name.trim(), url: url.trim(), description: description.trim() || undefined });
      
      // Reset Form
      setName('');
      setUrl('ws://localhost:2700');
      setDescription('');
      setTestResult(null);
