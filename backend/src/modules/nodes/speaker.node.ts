...
/**
 * SpeakerNode - Lautsprecher Output Node
 * Sendet Audio-Stream an ESP32-Client zur Wiedergabe
 */
export class SpeakerNode extends BaseNode {
  private readonly logger = new AppLogger('SpeakerNode');
  ...
  async start(): Promise<void> {
    this.isRunning = true;
    this.logger.info('Speaker node started', {
      nodeId: this.id,
  ...
  async process(uso: UniversalStreamObject, emitter: EventEmitter): Promise<void> {
    try {
      ...
      if (uso.header.type !== 'audio') {
        this.logger.warn('Speaker node received non-audio USO', {
          nodeId: this.id,
      ...
      if (!this.activeSession) {
        ...
        this.logger.info('Speaker node started playback session', {
          nodeId: this.id,
      ...
      // USO an Device senden (nur wenn Payload vorhanden)
      if (Buffer.isBuffer(uso.payload) && uso.payload.length > 0) {
        const sent = this.wsGateway.sendUSO(targetDevice, uso);

        if (!sent) {
          throw new Error(`Failed to send audio to device ${targetDevice}`);
        }

        this.logger.debug('Speaker node sent audio to device', {
          nodeId: this.id,
          sessionId: uso.header.id,
          targetDevice,
      ...
      if (uso.header.final) {
        this.logger.info('Speaker node playback completed', {
          nodeId: this.id,
          sessionId: this.activeSession,
          targetDevice,
        });
        ...
        // Optional: Sende Playback-Complete-Event an Device
        // (kann verwendet werden um z.B. LED auszuschalten)
      }
    } catch (error) {
      this.logger.error('Error in Speaker node processing', error.message, {
        nodeId: this.id,
        sessionId: uso.header.id,
      });
  ...
  async stop(): Promise<void> {
    // Wenn noch eine aktive Session läuft, sende Stop-Signal
    if (this.activeSession && this.config.deviceId) {
      // Optional: Sende Stop-Control-USO an Device
      this.logger.info('Stopping active playback', {
        nodeId: this.id,
        sessionId: this.activeSession,
      });
    }
    ...
    this.logger.info('Speaker node stopped', { nodeId: this.id });
  }
  ...
  getHealthStatus(): { status: 'healthy' | 'degraded' | 'error'; message?: string } {
    ...
    if (this.activeSession) {
      return {
        status: 'healthy',
        message: `Playing audio to ${this.config.deviceId}`,
      };
    }

    return {
      status: 'healthy',
      message: `Ready to play audio to ${this.config.deviceId}`,
    };
  }
}
